...
export class UpdateService {
  ...
  addVersionApplicabe(update: IUpdate): Observable<any> {
    ...
    return this.http.post<any>(
      ...
      { name: update.versionApplicable, comment: update.comment, description: update.description, productUid: update.product?.uidProduct },
  ...
  addVersionCible(update: IUpdate): Observable<any> {
    ...
    return this.http.post<any>(
      'https://test.actiaadn.com/api/v1/version/',
      { name: update.versionCible, comment: update.comment, description: update.description, productUid: update.product?.uidProduct },
      { headers: { authorization: token } }
    );
  }
  ...
  registerUpload(sizeFile: string, uidUpdate: string, contentFile: any, fileName: any): Observable<any> {
    ...
    const token: string | null = `${String(localStorage.getItem('PlatformAPIToken'))}`;
    return this.http.post<any>(
      `https://test.actiaadn.com/api/v1/upload/register/${uidUpdate}`,
      {
        name: fileName,
  ...
  EditStatut(update: IUpdate): Observable<any> {
    ...
    return this.http.patch<any>(
      `https://test.actiaadn.com/api/v1/update/${uiUpdate}`,
      {},
  ...
  getUpdate(update: IUpdate): Observable<any> {
    const token: string | null = `${String(localStorage.getItem('PlatformAPIToken'))}`;
    const uiUpdate: string = update.uidUpdate !== undefined ? update.uidUpdate : '';
    return this.http.get(`https://test.actiaadn.com/api/v1/update/${uiUpdate}`, { headers: { authorization: token } });
  }

  DeleteUpdateADN(update: IUpdate): Observable<any> {
    const token: string | null = `${String(localStorage.getItem('PlatformAPIToken'))}`;
    const uiUpdate: string = update.uidUpdate !== undefined ? update.uidUpdate : '';
    return this.http.delete<any>(`https://test.actiaadn.com/api/v1/update/${uiUpdate}`, { headers: { authorization: token } });
  }
  DeleteVersionApplicableADN(update: IUpdate): Observable<any> {
    const token: string | null = `${String(localStorage.getItem('PlatformAPIToken'))}`;
    const uidVer: string =
      update.versionApplicable?.uidVersionApplicable !== undefined ? update.versionApplicable?.uidVersionApplicable : '';
    return this.http.delete<any>(`https://test.actiaadn.com/api/v1/version/${uidVer}`, { headers: { authorization: token } });
  }
  DeleteVersionCibleADN(update: IUpdate): Observable<any> {
    ...
    return this.http.delete<any>(`https://test.actiaadn.com/api/v1/version/${uidVerCible}`, { headers: { authorization: token } });
  }
  ...
  EditerUpdate(update: IUpdate, statut: string): Observable<any> {
    ...
    return this.http.put<any>(
      `http://localhost:8080/api/updates/${idUpdate}`,
      {
        id: update.id,
