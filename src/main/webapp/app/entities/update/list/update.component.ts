...
export class UpdateComponent implements OnInit {
    ...
    this.updateService.getUpdate(update).subscribe(data => {
      this.ResGetUpdate = data.updateFiles
       ...
       if (this.ResGetUpdate.length===0) {
        ...
        this.updateService.EditStatut(update).subscribe(ResEditStatut => {
              //this.statut = ResEditStatut;
              this.updateNextStatut(ResEditStatut, update.id);
              this.updateService.EditerUpdate(update,ResEditStatut).subscribe(data1=>{console.log(data1)},error =>{ console.error(error)});
  ...
  delete(update: IUpdate): void {
    ...
    modalRef.closed.subscribe(reason => {
      if (reason === 'deleted') {
        this.loadAll();
        this.updateService.DeleteUpdateADN(update).subscribe(ResDelete=>{console.log(ResDelete)
          this.updateService. DeleteVersionApplicableADN(update).subscribe(ResDelVerApp=>{console.log(ResDelVerApp)})
